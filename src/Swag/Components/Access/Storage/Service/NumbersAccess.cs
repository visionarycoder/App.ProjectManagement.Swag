...
using Swag.Components.Access.Storage.Orm;
using Swag.Components.Access.Storage.Orm.Models;
using Swag.Ifx.Data.Helpers;

namespace Swag.Components.Access.Storage.Service;
...
public class NumbersAccess(ILogger<NumbersAccess> logger, DbContextOptions<NumbersContext> options) : INumbersAccess
{
    ...
    public async Task<bool> DeleteSwag(int id)
    {
        ...
        try
        {
            ...
            var entityEntry = ctx.Remove(new ThreePointEstimate { Id = id });
            var count = await ctx.SaveChangesAsync();
            return count != 0;
